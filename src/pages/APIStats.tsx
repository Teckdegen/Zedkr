...
`;
  ...
  return (
    <DashboardLayout>
        ...
        <div className="flex items-center gap-4 self-start md:self-auto">
          <div className="flex items-center gap-2 mr-2">
            <button
              ...
              title="Generate OpenClaw skill file"
            >
      ...
      {/* Direct Call Info Card */}
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.25 }}
        className="vercel-card p-6 mb-8 bg-primary/5 border border-primary/20"
      >
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h3 className="font-bold text-sm tracking-tight mb-2 flex items-center gap-2">
              <span className="text-primary">✨ Direct API Calls</span>
            </h3>
            <p className="text-xs text-zinc-400 mb-3">
              The easiest way to test your API - just add your private key to the URL or header. No code needed!
            </p>
            <div className="space-y-2 font-mono text-[10px] bg-zinc-950 p-3 rounded border border-white/5">
              <div className="text-zinc-500"># Method 1: Private key in URL</div>
              <div className="text-zinc-300">
                curl "{user?.username && (api.api_name_slug || api.apiNameSlug) && api.endpoints?.[0]
                  ? `https://zedkr.up.railway.app/${user.username}/${api.api_name_slug || api.apiNameSlug}/${api.endpoints[0].endpoint_path || api.endpoints[0].path}?privateKey=YOUR_PRIVATE_KEY`
                  : 'YOUR_ENDPOINT_URL?privateKey=YOUR_PRIVATE_KEY'}"
              </div>
              <div className="text-zinc-500 mt-2"># Method 2: Private key in header (recommended)</div>
              <div className="text-zinc-300">
                curl -H "x-private-key: YOUR_PRIVATE_KEY" "{user?.username && (api.api_name_slug || api.apiNameSlug) && api.endpoints?.[0]
                  ? `https://zedkr.up.railway.app/${user.username}/${api.api_name_slug || api.apiNameSlug}/${api.endpoints[0].endpoint_path || api.endpoints[0].path}`
                  : 'YOUR_ENDPOINT_URL'}"
              </div>
            </div>
            <p className="text-[10px] text-zinc-500 mt-3">
              💡 Payment is automatically processed via x402-stacks protocol. Perfect for testing, automation, and AI agents!
            </p>
          </div>
        </div>
      </motion.div>
